...
export async function analyzeImageWithClaude(req: Request, res: Response) {
  try {
    ...
    const result = await anthropic.messages.create({
      ...
      max_tokens: 1024,
      system: "You are a cryptocurrency expert that analyzes charts and images. When presented with a chart, describe the pattern and provide insights about possible price movements. When presented with logos or cryptocurrency-related images, identify the cryptocurrency and provide relevant information about it.",
      messages: [
        { 
          role: 'user', 
          content: [
            {
              type: "text",
              text: "Analyze this cryptocurrency-related image in detail and explain what you see."
            },
            {
              type: "image",
              source: {
